...
class Monad m => UpdateRepository m where
    loadData :: Text -> (CreateJurisdiction -> m(Either JurisdictionError Jurisdiction)) -> (CreateDepartment -> m(Either DepartmentError Department)) -> (CreateVaccine -> m(Either VaccineError Vaccine)) -> m(Either UpdateError Update)

update :: (UpdateRepository m, JurisdictionService m, DepartmentService m, VaccineService m, UpdateDao m,  MonadUnliftIO m) => Text -> m(Either UpdateError Update)
update url = do
...
deleteOldData :: (JurisdictionService m, DepartmentService m, VaccineService m) => m (Either UpdateError ())
deleteOldData = do 
    jurisdiction <- deleteJurisdictions
    department <- deleteDepartments
    vaccine <- deleteVaccines
    case lefts [mapEither jurisdiction, mapEither department, mapEither vaccine] of
        [] -> return $ Right ()
        _ -> return $ Left UnknownError

updateData :: (UpdateRepository m, JurisdictionService m, DepartmentService m, VaccineService m, UpdateDao m) => Text -> ZonedTime -> m(Either UpdateError Update)
updateData url t = do
    result <- loadData url createJurisdiction createDepartment createVaccine
